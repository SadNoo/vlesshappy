    ...
    public function getNodeList($request, $response, $args)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if ($this->user->class < $node->node_class) {
                ...
                $server = Tools::ssSinglePortArray($node->server);
                $array_node['server'] = $server['add'];
            } elseif ($node->sort == VlessReality::NODE_SORT) {
                $array_node['server'] = VlessReality::publicEndpoint($node);
            } else {
            ...
            if ($node->sort == VlessReality::NODE_SORT) {
                $safeNode = clone $node;
