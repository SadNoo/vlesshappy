...
use App\Services\{Config, AppsProfiles, VlessReality};
...
class LinkController extends BaseController
{
    ...
    public static function GetContent($request, $response, $args)
    {
        ...
        $isVless = isset($opts['vless'])
            && (string) $opts['vless'] === '1'
            && preg_match_all('/(?:^|&)vless=/', $queryString) === 1;
        if (isset($opts['vless']) && !$isVless) {
            $opts['vless'] = '0';
        }
        ...
        $sub_type_array = ['list', 'vless', 'ss2022', 'ssd', 'clash', 'surge', 'sing-box', 'loon', 'kitsunebi', 'surfboard', 'quantumult', 'quantumultx', 'shadowrocket', 'sub'];
    ...
    public static function getClash($user, $clash, $opts, $Rule)
    {
        ...
        // 只有显式 ?clash=1 才聚合 VLESS；兼容入口 ?mu=4 保持原语义。
        if ((string) $clash === '1' && !isset($opts['mu'])) {
            $usedNames = [];
            foreach ($Proxys as $proxy) {
                $usedNames[(string) $proxy['name']] = true;
            }
            foreach (VlessReality::mihomoProxies($user) as $proxy) {
                $proxy['name'] = self::getUniqueProxyName($proxy['name'], $usedNames);
                $Proxys[] = $proxy;
            }
        }
        if (isset($opts['source']) && $opts['source'] != '') {
    ...
    private static function getUniqueProxyName($name, &$usedNames)
    {
    ...
    /**
     * 仅包含 Shadowsocks 2022 节点的 SIP002 订阅。
     */
    public static function getSS2022($user, $ss2022, $opts, $Rule)
    {
        $Rule['type'] = 'ss';
        $Rule['is_mu'] = 0;
        ...
        foreach ($nodes as $item) {
            if (
                $item['type'] != 'ss'
                || strpos($item['method'], '2022-') !== 0
            ) {
                continue;
            }
            $link = URL::getItemUrl($item, 1);
            if ($link !== null && $link != '') {
                $links[] = $link;
            }
        }

        return base64_encode(implode(PHP_EOL, $links));
    }

    /**
     * 仅包含 VLESS REALITY 节点的标准 URI 订阅。
     */
    public static function getVless($user, $vless, $opts, $Rule)
    {
        if ((string) $vless !== '1') {
            return '';
        }
        return VlessReality::subscription($user);
    }

    /**
     * Loon 托管配置，包含该客户端支持的 SS/SS2022、SSR 和 VMess 节点。
     */
    public static function getLoon($user, $loon, $opts, $Rule)
    {
        $Rule['type'] = 'all';
        $nodes = URL::getNew_AllItems($user, $Rule);
        $proxies = [];
        ...
        foreach ($nodes as $item) {
            $item['remark'] = self::getUniqueProxyName(
                AppURI::normalizeLoonName($item['remark']),
                $usedNames
            );
            $proxy = AppURI::getLoonURI($item);
            if ($proxy === null) {
                continue;
            }
            $proxies[] = $proxy;
            $proxyNames[] = $item['remark'];
        }

        return ConfController::getLoonConfs(
            self::getSubinfo($user, 0)['loon'],
            $proxies,
            $proxyNames
        );
    }
    ...
    /**
     * sing-box 完整配置，包含该客户端支持的 SS/SS2022、VMess 和 VLESS 节点。
     */
    public static function getSingBox($user, $singBox, $opts, $Rule)
    {
        $Rule['type'] = 'all';
        $nodes = URL::getNew_AllItems($user, $Rule);
        $proxyOutbounds = [];
        ...
        foreach ($nodes as $item) {
            $item['remark'] = self::getUniqueProxyName($item['remark'], $usedNames);
            $outbound = AppURI::getSingBoxURI($item);
            if ($outbound === null) {
                continue;
            }
        ...
        foreach (VlessReality::singBoxOutbounds($user) as $outbound) {
            $outbound['tag'] = self::getUniqueProxyName($outbound['tag'], $usedNames);
            $proxyOutbounds[] = $outbound;
            $proxyNames[] = $outbound['tag'];
        }

        return ConfController::getSingBoxConfs($proxyOutbounds, $proxyNames);
    }
